...
function WikidataValidate(props) {
  ...
  function statusReducer(status, action) {
    switch (action.type) {
      ...
      case "set-params":
        ...
        const es = params.node
          ? params.node.split(",").map((node) => {
              return { uri: node };
            })
          : [];
      ...
      case "set-schemaEntity":
        console.log(
          `statusReducer: set-schemaEntity: ${JSON.stringify(action.value)}`
        );
  ...
  async function validate() {
    console.log(
      `Validate: entities: ${JSON.stringify(
        status.entities
      )}, shape: ${JSON.stringify(status.shapeLabel)}`
    );
    ...
    else paramsShEx = paramsFromShEx(shEx);
    ...
    dispatch({
      type: "set-permalink",
      value: mkPermalinkLong(API.wikidataValidateRoute, paramsPermalink),
    });
    ...
    <Container>
      ...
      <Row>
        <Form onSubmit={handleSubmit}>
          ...
          />
          <Tabs
            activeKey={status.schemaActiveTab}
            transition={false}
            id="SchemaTabs"
